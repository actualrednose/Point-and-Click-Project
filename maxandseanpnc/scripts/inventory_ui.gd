...
func _build_ui() -> void:
	...
	_panel.add_theme_stylebox_override("panel", _text_sb)  # Empty = invisible.
...
func _set_open(value: bool) -> void:
	...
	if _open:
		...
		_panel.size = natural          # Full size, laid out completely.
		_clip.position = Vector2(MARGIN + BAG_SIZE.x + GAP,
...
func _target_width() -> float:
	...
	return minf(natural_x, SCREEN.x - (MARGIN + BAG_SIZE.x + GAP) - MARGIN)

func _tween_clip(target: float, time: float, hide_when_done: bool, springy: bool) -> void:
	if _size_tween and _size_tween.is_valid():
		_size_tween.kill()
	_size_tween = create_tween()
	var step := _size_tween.tween_property(_clip, "size:x", target, time)
	if springy:
		step.set_trans(Tween.TRANS_BACK).set_ease(Tween.EASE_OUT)
	else:
		step.set_trans(Tween.TRANS_CUBIC).set_ease(Tween.EASE_IN)
	if hide_when_done:
		_size_tween.tween_callback(func() -> void: _clip.visible = false)

## Re-run after slot count changes while open (deferred one frame so
## the container's minimum size includes the change).
...
func _apply_bag_icon() -> void:
	if _bag_tex_closed:
		var tex: Texture2D = _bag_tex_open if (_open and _bag_tex_open) else _bag_tex_closed
		_bag.icon = tex
		_bag.expand_icon = true
		_bag.text = ""
		_bag.rotation = 0.0
		_bag.add_theme_stylebox_override("normal", _bag_sb_open if _open else _bag_sb_normal)
		...
		_bag.text = "Bag"
		_bag.rotation = 0.12 if _open else 0.0  # Lean while open as the cue.
...
func _pop_bag() -> void:
	...
	tw.tween_property(_bag, "scale", Vector2(0.92, 0.92), 0.08)
	tw.tween_property(_bag, "scale", Vector2.ONE, 0.12) \
...
func _add_slot(item: ItemDef, animate: bool = true) -> void:
	...
	if animate:
		...
		tw.tween_property(btn, "scale", Vector2.ONE, 0.22) \
			.set_trans(Tween.TRANS_BACK).set_ease(Tween.EASE_OUT)
		tw.parallel().tween_property(btn, "modulate:a", 1.0, 0.12)

## An icon slot: TextureButton wearing the same glow shader the
## world interactables use. Hover = gold outline, exactly like them.
func _make_icon_slot(item: ItemDef) -> TextureButton:
	...
	mat.set_shader_parameter("hovered", 0.0)  # Seed the animated uniform.
	btn.material = mat
...
## Glow target = selected OR hovered. Selection pins the glow on;
## hover alone still glows unselected items as before.
func _update_glow(item: ItemDef) -> void:
	var btn: Control = _slot_by_item.get(item)
	if btn == null:
		return
	var on: bool = Inventory.selected_item == item or _hovered_items.has(item)
	var tw: Tween = _hover_tweens.get(item)
	if tw and tw.is_valid():
		tw.kill()
	if btn.material is ShaderMaterial:
		# Icon slot: fade the gold outline via set_shader_parameter —
		# immune to how the material was built.
		...
		if absf(target - from) < 0.001:
			return  # Already there — no pointless tween.
		tw = btn.create_tween()
		tw.tween_method(
			func(v: float) -> void: mat.set_shader_parameter("hovered", v),
			from, target, 0.15)
	else:
		# Text fallback: gold tint stands in for the glow.
		var target_color := SELECTED_TINT if on else Color.WHITE
...
func _on_item_added(item: ItemDef) -> void:
	_add_slot(item)
	if _open:
		_retarget_width()  # Bar visibly stretches to fit the new slot.
	else:
		_pop_bag()  # Closed? The bag wiggles: "it went in".
...
func _on_item_removed(item: ItemDef) -> void:
	...
	if btn:
		...
		_hovered_items.erase(item)  # Freed slots can't mouse_exit.
	if Inventory.items.is_empty():
		_set_open(false)  # Last item used — the bar folds itself away.
	elif _open:
		_retarget_width()
