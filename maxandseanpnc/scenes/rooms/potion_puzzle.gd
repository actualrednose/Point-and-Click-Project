...
## in the inventory — the mixing table brews one mixture at a time.
...
var _closeup: Node
var _cup_contents := ""
# "", "red", "yellow", "blue" (mixed potions leave immediately)
...
func open_closeup() -> void:
	...
	if closeup_scene == null:
		if ResourceLoader.exists(FALLBACK_CLOSEUP):
			...
			push_warning(
				"PotionPuzzle: closeup_scene not wired and %s doesn't exist."
				% FALLBACK_CLOSEUP
			)
			return
...
func vials_unlocked() -> bool:
	if key_theft == null:
		push_warning(
			"PotionPuzzle: key_theft export is empty — the vials stay locked. "
			+ "Drag lab.tscn's KeyTheft node in."
		)
		return false
...
func _on_flask_clicked(color: String) -> void:
	...
	if vial == null:
		_closeup.say(
			"The blue flask is bone dry. I'd need a replacement vial..."
		)
		return

	Inventory.remove_item(vial)
	# auto-deselects if somehow held

	RoomState.set_flag(self, "blue_refilled", true)
	blue_flask_refilled.emit()
	# room-scale art swaps behind the overlay
...
func _on_pour_requested(color: String) -> void:
	...
	_kill_auto_close()
	# a fresh pour cancels any pending auto-close
...
func _mix(second: String) -> void:
	...
	var result: String = MIX_RESULTS.get(
		pair[0] + "+" + pair[1],
		""
	)
	...
	if result.is_empty() or def == null:
		push_warning(
			"PotionPuzzle: the %s potion ItemDef isn't assigned — "
			+ "drag its .tres into the exports."
			% [
				result if not result.is_empty() else "mixed"
			]
		)
	...
	# Display-only: the cup shows the brewed color as we leave.
	_closeup.set_cup(result)
...
func drink_potion(item: ItemDef) -> void:
	if _effect_busy or _entering:
		_say(
			"Maybe let my body finish the last experiment first...",
			_player_anchor()
		)
		return
...
func _effect_grow(item: ItemDef) -> void:
	...
	# Wait one second after returning to normal size before explaining
	# why the orange potion cannot be used inside.
	tw.tween_interval(1.0)
	tw.tween_callback(_grow_return_comment)
...
func _grow_return_comment() -> void:
	_say(
		"Ouch. That one's definitely too strong to use inside at the moment.",
		_player_anchor()
	)
...
func _effect_shrink(item: ItemDef) -> void:
	...
	if player_is_small():
		...
		return
	# potion stays in the bag — not wasted
...
func _shrink_arrived() -> void:
	...
	_say(
		"Whoa — everything's HUGE. That mousehole looks door-sized now.",
		_player_anchor()
	)
...
func enter_hole(hole: Node2D) -> void:
	...
	if p.has_method("set_facing"):
		p.set_facing(
			1 if hole.global_position.x >= p.global_position.x else -1
		)

	_say(
		"(Here goes nothing...)",
		hole.global_position - Vector2(0.0, 60.0)
	)
	...
	tw.tween_callback(
		_on_reached_hole.bind(p, saved_speed)
	)
...
func _on_reached_hole(player: Node2D, saved_speed: float) -> void:
	...
	if not next_room_path.is_empty():
		...
		return
	# this room dies here — touch nothing afterwards

	# No next room yet: peek, fade back in, stay tiny at the hole mouth.
	_say(
		"(It's pitch dark in there... the next room hasn't been built yet.)",
...
func drink_and_enter(hole: Node2D, item: ItemDef) -> void:
	...
	if player_is_small():
		enter_hole(hole)
		# already tiny — straight to the climb (potion kept)
		return
...
func _shrink_for_hole() -> void:
	...
	_say(
		"Everything's HUGE... and that hole looks door-sized.",
		_player_anchor()
	)
...
func _player_anchor() -> Vector2:
	...
	return p.global_position + Vector2(
		0.0,
		-110.0 * p.scale.y - 60.0
	)


## Appends a feet-preserving scale step to a tween chain. The player's
## origin sits mid-body, so raw scaling would sink/lift the feet into the
## floor — position.y is compensated every frame to keep them planted.
func _tween_scale(
	tw: Tween,
	target: Vector2,
	time: float,
	...
	tw.tween_method(
		_set_scaled.bind(_scale_from, target),
		0.0,
		1.0,
		time
